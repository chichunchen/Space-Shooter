...
public class StageChooseCraft extends JPanel implements ActionListener
{
	...
	public StageChooseCraft( JFrame mframe )
	{
		setLayout( null );
		...
		nameLable = new JLabel( "Stage Mode" );
		...
		for (int i = 0; i < craftButton.length; i++) 
		{
			if(event.getSource() == craftButton[i]) 
			{
				if(playerName.length() > 0 && playerName.equals("Your name") == false) 
				{
					...
					frame.add( new StageMode(playerName, i+1, frame) );
